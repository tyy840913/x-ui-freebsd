...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://s.wosk.asia/api.github.com/repos/parentalclash/x-ui-freebsd/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        last_version=$1
        url="https://s.wosk.asia/github.com/vaxilu/x-ui/releases/download/${last_version}/x-ui-${release}-${arch}.tar.gz"
